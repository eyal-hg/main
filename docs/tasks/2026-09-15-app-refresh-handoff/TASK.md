...
> **עדכון 15.09, אחרי סקירה נוספת:** נוסף קומיט שמיני `e5f57fe` (ראש הענף החדש), שלושה קבצים: `ClientDetailsScreen.tsx` — טופס "קביעת פגישה"/"שינוי מועד" בכרטיס כבר לא מתאפס בכל רענון של הכרטיס (פעם בדקה), ו"קביעה ›" מושבת בזמן שהקלטה נפתחת; `ClientAssistant.tsx` — נעילה נגד שליחה כפולה, בלי ערבוב בין שיחות כשעוברים באמצע טעינה או שליחה, שגיאה בשליחה חוזרת לא נבלעת, שיחה חדשה מופיעה מיד ב"שיחות קודמות", שגיאת טעינת שיחה עם "ניסיון נוסף", היסט המקלדת בלי גובה שורת השיחה, שיחה ריקה נמחקת כשיוצאים מיד; `Markdown.tsx` — קו מפריד אחרי רשימה או ציטוט, עשרוניים בגרף, `***מודגש נטוי***`, תווי בריחה. tsc 0 שגיאות, lint בלי חדשות. בבדיקות סעיף ה לבדוק גם את אלה.
