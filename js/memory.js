...
let MEM_CATS=[
  /* ===== חברה ===== */
  {key:'cashpos', scope:'company', name:'מצב תזרימי', internal:false, vis:'client', lastRun:'02.07',
...
אם עלה: עדכן את המסמך כתמונת מצב בזמן הווה, עד 8 שורות. החלף נתונים שהשתנו — אל תנהל יומן אירועים ואל תוסיף שורות מצטברות. לכל קביעה מהותית צרף (תאריך · מקור). עובדות תזרים בלבד — בלי פרשנות רגשית.`},
  {key:'bizprofile', scope:'company', name:'פרופיל עסקי', internal:false, vis:'client', lastRun:'15.06',
...
אם אין — החזר "אין עדכון". אם יש: עדכן את המסמך (עד 10 שורות) כתיאור עדכני של העסק. החלף עובדות שהשתנו. זה המסמך שממנו ה-AI מבין את העסק — כתוב יבש, מדויק וללא הערכות.`},
  {key:'pains', scope:'company', name:'כאבי לקוח', internal:false, vis:'hk', lastRun:'02.07',
...
אם אין שינוי — החזר "אין עדכון". אם יש: עדכן את הרשימה — עד 6 כאבים ממוינים לפי חומרה, שורה לכאב: הכאב, מאז מתי, (תאריך · מקור). כאב שנפתר — הסר, או השאר שורה אחת "נפתר (תאריך)" למשך חודש.`},
  {key:'goals', scope:'company', name:'יעדים והסכמות', internal:false, vis:'client', lastRun:'02.07',
...
אם אין — החזר "אין עדכון". אם יש: עדכן — שורה ליעד: היעד, בעל האחריות, מועד, סטטוס, (תאריך · מקור). יעד שהושלם לפני יותר מחודש — הסר.`},
  {key:'events', scope:'company', name:'אירועים מהותיים', internal:false, vis:'client', lastRun:'20.06',
...
אם אין — החזר "אין עדכון". אם יש: עדכן — עד 5 אירועים, כל אחד עם תאריך והשפעה צפויה במשפט. הסר אירועים שמוצו.`},
  {key:'coop_co', scope:'company', name:'שיתוף פעולה', internal:false, vis:'client', lastRun:'02.07',
...
אם אין שום מידע חדש — החזר "אין עדכון". אחרת עדכן תמונת מצב: מה עובד, מה נתקע ומאז מתי, עם (תאריך · מקור). עד 6 שורות. מדד תפעולי — בלי שיפוטיות על אנשים.`},
  /* ===== יוזר ===== */
  {key:'commstyle', scope:'user', name:'סגנון תקשורת', internal:false, vis:'hk', lastRun:'02.07',
...
אם אין — החזר "אין עדכון". אם יש: עדכן — נסח כהנחיות פעולה לצ'אט ("פתח במספרים", "דבר קצר", "אל תציף בפרטים"), עד 6 הנחיות. המסמך הזה מוזן לצ'אט הלקוח — אסור לכלול בו תצפיות רגשיות או ציטוטים, רק הנחיות טון.`},
  {key:'csat', scope:'user', name:'שביעות רצון מהשירות', internal:true, vis:'hk', lastRun:'02.07',
...
המסמך עצמו לעולם לא נשלח לצ'אט. בסוף כל עדכון הפק גם שדה נפרד "הנחיות לצ'אט" — עד 2 הנחיות התנהגות מזוקקות בלי אף עובדה מהמסמך (למשל: "הקפד על מענה מהיר ומדויק — יש רגישות לאיכות השירות"). רק השדה הזה מוזן לצ'אט.`},
  {key:'mood', scope:'user', name:'מצב אישי ולחץ', internal:true, vis:'hk', lastRun:'02.07',
...
המסמך עצמו לעולם לא נשלח לצ'אט. בסוף כל עדכון הפק גם שדה נפרד "הנחיות לצ'אט" — עד 2 הנחיות טון מזוקקות בלי אף עובדה (למשל: "ענה ברוגע ובקצרה, הצג תמיד פתרון לצד בעיה"). רק השדה הזה מוזן לצ'אט; תשובת הצ'אט עוברת בדיקת guard שחוסמת אזכור תוכן רגיש.`},
  {key:'coop_user', scope:'user', name:'שיתוף פעולה אישי', internal:false, vis:'client', lastRun:'02.07',
...
אם אין — החזר "אין עדכון". אם יש: עדכן — מה הוא מיישם, איפה נדרש חיזוק, ואיך הכי אפקטיבי להניע אותו. עד 5 שורות עם (תאריך · מקור).`},
  {key:'empfb', scope:'user', name:'משוב ומחמאות לעובד', internal:false, vis:'client', appliesTo:'employee', lastRun:'02.07',
...
אם אין — החזר "אין עדכון". אם יש: עדכן — תמונת משוב עדכנית עם (תאריך · מקור), עד 5 שורות. גלוי לאדמין הלקוח — נסח מקצועי ומאוזן, בלי ציטוטים רגישים.`},
  {key:'relation', scope:'user', name:'הקשר אישי ואמון', internal:false, vis:'hk', lastRun:'25.06',
...
let MEM_CUR=0, MEM_USER=0, MEM_EDIT=null, MEM_HIST=new Set();
function openMemCard(i){
  MEM_CUR=i; MEM_USER=0; MEM_EDIT=null; MEM_HIST=new Set();
  renderMemCard();
...
function memCatBlock(cat){
  ...
  return `<div class="mem-cat ${cat.internal?'internal':''}">
    <div class="mem-cat-h">
      <b>${cat.name}</b>
      ${cat.vis==='client'?'<span class="mem-vis adm">גלוי גם ללקוח</span>':'<span class="mem-int">צוות HK בלבד</span>'}
...
function renderMemCard(){
  ...
  document.getElementById('memTitle').innerHTML=`<span class="ap-av" style="width:36px;height:36px;font-size:15px">${c.name.charAt(0)}</span>
  ...
  document.getElementById('memBody').innerHTML=`${nums}`;
...
function renderUserSet(){
  ...
  document.getElementById('userSetBody').innerHTML=`
    ...
    ${cats.map(c=>`<label class="us-cat">
      ...
      <b>${c.name}</b><span>${c.vis==='client'?'גלוי גם ללקוח':'צוות HK בלבד'}</span>
    </label>`).join('')}`;
}
...
let MEMADM_SEL=null;
function openMemAdmin(){ MEMADM_SEL=null; renderMemAdmin(); document.getElementById('memAdmOv').classList.add('show'); }
function closeMemAdmin(){ document.getElementById('memAdmOv').classList.remove('show'); }
function renderMemAdmin(){
  const row=(c,i)=>`
    <div class="ma-row2 ${MEMADM_SEL===i?'on':''}" onclick="MEMADM_SEL=${MEMADM_SEL===i?null:i};renderMemAdmin()">
      ...
      <b class="ma-nm">${c.name}</b>
      <span class="ma-vis ${c.vis==='client'?'client':'hk'}">${c.vis==='client'?'גלוי גם ללקוח':'צוות HK בלבד'}</span>
      <span>${c.appliesTo==='employee'?'<span class="ma-app">עובדים בלבד</span>':''}</span>
    ...
    ${MEMADM_SEL===i?`<div class="ma-prompt">
      <div class="ma-plbl">הפרומפט — נשלח ל-flow אחרי כל אינטראקציה יחד עם המסמך הנוכחי</div>
      <textarea class="mem-ta" id="maTa_${i}" rows="8">${c.prompt}</textarea>
      <div class="mem-editfoot"><button class="ot-btn done" onclick="MEM_CATS[${i}].prompt=document.getElementById('maTa_${i}').value;MEMADM_SEL=null;renderMemAdmin();toast('הפרומפט עודכן')">שמירת הפרומפט</button></div>
    </div>`:''}`;
  const grp=(title,scope)=>{
    ...
    return `<div class="ma-grp">
      ...
      <div class="ma-row2 head"><span></span><b>קטגוריה</b><span>מי רואה בממשק</span><span>תחולה</span><span>עדכון אחרון</span><span></span></div>
  ...
  document.getElementById('memAdmBody').innerHTML=rows+`
    <div class="ma-new">
      <div class="ma-plbl big">קטגוריה חדשה</div>
      ...
      <div class="ma-opts">
        ...
        <div class="ma-opt">
          <label>מי רואה בממשק</label>
          <span class="cr-modes" id="maNewVis"><span class="mtk-chip on" data-v="hk" onclick="maNewVisSet(this)">צוות HK בלבד</span><span class="mtk-chip" data-v="client" onclick="maNewVisSet(this)">גלוי גם ללקוח</span></span>
        </div>
      </div>
      <div class="ma-plbl">הפרומפט — מה לבדוק באינטראקציה, מתי לעדכן, כללי כתיבה וגבולות</div>
      <textarea class="mem-ta" id="maNewPrompt" rows="4" placeholder="אתה מתחזק את קטגוריית… בדוק אם… אם אין — החזר ״אין עדכון״. אם יש — עדכן…"></textarea>
      <div class="mem-editfoot"><button class="ot-btn done" onclick="memAdmAdd()">הוספת הקטגוריה</button></div>
    </div>`;
}
function maNewScope2(el){
  ...
  document.getElementById('maNewAppWrap').style.display=el.dataset.s==='user'?'':'none';
}
