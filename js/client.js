/* HK Dashboard — client extras: AI chat orb, document uploads, operator activity feed */
  ...
  function aiAnswer(q){
    ...
    if(q.includes('חריגה')||q.includes('יולי')){
      return {t:'צפויה חריגה של ‎-289,161 ₪ ב-13.7 בעו״ש המאוחד. הגורם המרכזי: הוצאות השכר חרגו ב-14% מהתקציב החודשי, ובמקביל תשלום ספק גדול (210k) מתוכנן ל-14.7. מנהל התזרים כבר קיבל התראה — מומלץ לתאם שיחה עם היועץ.',
    ...
    if(q.includes('מתפעל')||q.includes('HK')){
      return {t:'השבוע אצל מנהל התזרים הושלם תפעול מלא (32 דק׳), קוטלגו 4 תנועות חדשות, טופלה חשבונית סונול, נשלח אליך עדכון תזרים והוסר חיוב כפול של 3,540 ₪. הפירוט המלא בכרטיס "מה מנהל התזרים עשה עבורך החודש".'};
    }
    if(q.includes('פגישה')){
