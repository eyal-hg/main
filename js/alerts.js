/* HK Dashboard — metric alert engine, alerts focus (advisor/HK), health map */
  ...
  const ADV_PREPS={
    /* שתי הנקודות האחרונות נולדו מסיווג הפער בתקציב — "לא צפוי להתממש" ו"היעד לא מעודכן"
       הן היחידות שדורשות הסבר בשורה אחת, ובדיוק ההסבר הזה עולה לכאן. */
    'אנרגי אינטרנשיונל':{pts:['ההכנסות +12% מהחודש הקודם — כדאי לפתוח בזה','חריגה צפויה בעו״ש בעוד 9 ימים — להציע העברה מפועלים 112','חברות שילוח לא ייסגר החודש (2,553 ₪) — הלקוח איבד את מוסך רם, ההסכם לא יחודש','יעד פרסום ושיווק לא מעודכן — נקבע לפני שהקמפיין בגוגל הופסק, כדאי לעדכן יחד'],
