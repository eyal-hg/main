/* HK Dashboard — metric alert engine, alerts focus (advisor/HK), health map */
  ...
  /* ההכנה חיה בחדר הפגישה (מצב 1) — לא במסך נפרד */
  function goPrep(i){selectClient(i);showTab('meetings');}
  function advPulse(c){
  ...
  /* חריגה בפועל של לקוח — לרשימת הלקוחות בסרגל מנהל התזרים */
  window.riskOf=function(name){ return ADV_RISK.find(r=>r.c===name)||null; };
  let ADV_MGR_TASKS=[
    {d:'28.07', mgr:'טל מלקר', client:'מטעי גבעון', t:'עדכון שורה תקציבית — קניות מלאי', detail:'היעד החודשי חצה 114% — לעדכן את השורה מול הלקוח ולצבוע מחדש בתזרים.', open:true},
    ...
    {d:'01.08', mgr:'טל מלקר', client:'אנרגי אינטרנשיונל', t:'צביעת יתרת קניות מלאי בתזרים', detail:'נותרו 25,000 ₪ שלא נצבעו — להזין לתזרים קדימה.', open:true},
  ...
  const ADV_PREPS={
    ...
    'אנרגי אינטרנשיונל':{pts:['ההכנסות +12% מהחודש הקודם — כדאי לפתוח בזה','חריגה צפויה בעו״ש בעוד 9 ימים — להציע העברה מפועלים 112','המציאות השתנתה בחברות שילוח (2,553 ₪) — מוסך רם עזב, ההסכם לא יחודש','טעינו בבנייה בפרסום ושיווק — ההערכה התבססה על רבעון חזק; לתקן בבניית שלושת החודשים הבאים'],
